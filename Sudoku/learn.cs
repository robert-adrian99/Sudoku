...
namespace WindowsFormsApplication6
{
    public partial class learn : Form
    {
        ...
        private string lvl = "learn", nickname;

        private TextBox[,] matrix = new TextBox[4, 4];

        private SudokuClass b = new SudokuClass(4);

        private int sec = 0, mins = 0, yes_no = 0;

        public learn(string s,int ok)
        {
        ...
        private void learn_Load(object sender, EventArgs e)
        {
            ...
            Font font = new Font("Times New Roman", 44.0f);
            ...
            for (int i = 0; i < table.RowCount; i++)
            {
                for (int j = 0; j < table.ColumnCount; j++)
                {
                    ...
                    matrix[i, j].Location = new Point(i * 90, j * 90);
                    matrix[i, j].AutoSize = false;
                    matrix[i, j].Width = 90;
                    matrix[i, j].Height = 90;
        ...
        private void AddNumbers()
        {
            ...
            while (ok < 8)
            {
                ...
                int x1 = rnd1.Next(0, 4);
                int x2 = rnd2.Next(0, 4);
        ...
        private void table_Paint(object sender, PaintEventArgs e)
        {
            ...
            Rectangle rect = new Rectangle(0, 0, 381, 333);

            e.Graphics.DrawRectangle(blackPen, rect);

            e.Graphics.DrawLine(blackPen, 0F, 166F, 500F, 166F);

            e.Graphics.DrawLine(blackPen, 190F, 0F, 190F, 400F);
        ...
        private void verify(object sender, EventArgs e)
        {
            ...
            if (ok == 1)
            {
                ...
                //else
                //{
                //    string b = @"UPDATE board SET Mins = '" + mins + "',Sec = '" + sec + "', Date = GETDATE() WHERE Nickname = '" + nickname + "'";
                //    cmd = new SqlCommand(b, con);
                //    cmd.ExecuteNonQuery();
                //}
        ...
        private void matrix_TextChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < table.RowCount; i++)
                for (int j = 0; j < table.ColumnCount; j++)
                    if (matrix[i, j].Text != "" && "1234".IndexOf(char.Parse(matrix[i, j].Text)) < 0)
                    {
                        MessageBox.Show("Please, enter just numbers from 1 to 4!");
                        matrix[i, j].Text = "";
                    }
            verify(sender, e);
            ...
            for (int i = 0; i < table.RowCount; i++)
                for (int j = 0; j < table.ColumnCount; j++)
                    if (matrix[i, j].Location == textClicked.Location)
                    {
                        ...
                        for (int k = 0; k < 2; k++)
                            for (int l = 0; l < 2; l++)
                                matrix[i / 2 * 2 + k, j / 2 * 2 + l].BackColor = Color.LightGray;
                        break;
                    }
